...
if [ -z $SETENV_SETTED ]; then
    ...
    if [[ ! -f ${APP_HOME}/target/${APP_NAME}/bin/appctl.sh ]]; then
        ...
        let XmsValue=$memTotal*60/100
        let XmnValue=XmsValue/2
        echo "Xms_Value: "${XmsValue}""
        SERVICE_OPTS="${SERVICE_OPTS} -Xms${XmsValue}m -Xmx${XmsValue}m"
        ...
        echo "GC_TYPE: "${GC_TYPE}""

        if [[ "$GC_TYPE" == "G1" ]]; then
          SERVICE_OPTS="${SERVICE_OPTS} -XX:+UseG1GC"
          SERVICE_OPTS="${SERVICE_OPTS} -XX:InitiatingHeapOccupancyPercent=40"
          SERVICE_OPTS="${SERVICE_OPTS} -XX:-G1UseAdaptiveIHOP"

          G1HeapRegionSizeValue=4
          if [[ "$XmsValue" -le 2048 ]]; then
              G1HeapRegionSizeValue=4
          elif [[ "$XmsValue" -le 4096 ]]; then
              G1HeapRegionSizeValue=8
          elif [[ "$XmsValue" -le 8192 ]]; then
              G1HeapRegionSizeValue=16
          elif [[ "$XmsValue" -gt 8192 ]]; then
              G1HeapRegionSizeValue=32
          fi
          SERVICE_OPTS="${SERVICE_OPTS} -XX:G1HeapRegionSize=${G1HeapRegionSizeValue}m"
          echo "G1HeapRegionSize_Value: "${G1HeapRegionSizeValue}""
          ...
          SERVICE_OPTS="${SERVICE_OPTS} -Xmn${XmnValue}m"
        fi

        if [[ "$JAVA_VERSION" -ge 9 ]]; then
          SERVICE_OPTS="${SERVICE_OPTS} --add-opens java.management/java.lang.management=ALL-UNNAMED --add-opens java.management/sun.management=ALL-UNNAMED"
        fi

        if [[ "$JAVA_VERSION" -ge 17 ]]; then
          SERVICE_OPTS="${SERVICE_OPTS} --add-opens java.base/java.lang=ALL-UNNAMED --add-opens java.base/sun.net.util=ALL-UNNAMED"
        fi
        ...
        if [[ "$SERVICE_OPTS" != *"-Dspring.profiles.active"* ]] && [[ -n "$envSign" ]]; then
          if [ "$envSign" = "project" ] || [ "$envSign" = "test-trunk" ]; then
            SERVICE_OPTS="${SERVICE_OPTS} -Dspring.profiles.active=testing"
          else
            SERVICE_OPTS="${SERVICE_OPTS} -Dspring.profiles.active=$envSign"
          fi
        fi
        # debug opts

        # jpda options
        # Warning!!!set default value, please don't edit the default value.
        # if you want to enable java debug, go to dockerfile add `ENV JPDA_ENABLE=1`
        # 警告！！！设置JDPA的默认值，请不要更改默认值。
        # 如果你期望打开JDPA，请前往dockerfile添加：ENV JPDA_ENABLE=1
