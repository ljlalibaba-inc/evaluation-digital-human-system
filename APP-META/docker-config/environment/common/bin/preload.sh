                    ...
                    if [ -r "$SERVICE_PID" ]; then
                        ...
                        if [ $? -eq 0 ] ; then
                            if [[ "$JAVA_VERSION" -lt 9 ]]; then
                              CP_STR="${APP_HOME}/target/${APP_NAME}:${JAVA_HOME}/lib/tools.jar"
                            else
                              CP_STR="${APP_HOME}/target/${APP_NAME}"
                            fi
                            ret=$($JAVA_HOME/bin/java -classpath "$CP_STR" \
                                    -Dproject.name="${APP_NAME}" -Dpid="$PID" \
                                    "com.taobao.pandora.boot.loader.jmx.Executor" preload 2>&1 | tee -a ${APP_HOME}/logs/${APP_NAME}_deploy.log | \
